      ...
      <div>
        ...
        <ul className="flex flex-col gap-5">
          {Array.from({ length: placeholderCount }).map((_, index) => (
            <li
              className="flex flex-col items-center gap-x-5 gap-y-4 @container @sm:flex-row"
              key={index}
            >
              {/* Image */}
...
export function CartEmptyState({ title, subtitle, cta }: CartEmptyState)
{
  return (
    <SectionLayout className="text-center">
      <h1 className="mb-3 text-white text-center font-heading text-3xl leading-none text-foreground @xl:text-4xl">
        {title}
      </h1>
