...
}: Props<LineItem>)
{
  ...
  return (
    <StickySidebarLayout
      sidebar={
        <div>
          <h2 className="mb-10 font-heading text-white text-4xl font-medium leading-none @xl:text-5xl">
            {summaryTitle}
          </h2>
          <dl aria-label="Receipt Summary" className="w-full">
            <div className="divide-y divide-[#424242]">
              {cart.summaryItems.map((summaryItem, index) => (
                <div className="flex justify-between py-4 text-white" key={index}>
            ...
            <div className="flex justify-between border-t border-[#424242] py-6 text-white text-xl font-bold">
          ...
          <div className='flex flex-col items-center gap-y-4'>
            <CheckoutButton action={checkoutAction} className="mt-4 flex-1 w-full">
              <LockIcon key="lock" />
              {checkoutLabel}
    ...
    >
      <div className="w-full">
        <h1 className="mb-10 font-heading text-white text-4xl font-medium leading-none @xl:text-5xl">
          {title}
        ...
        <ul className="flex flex-col gap-5">
          {optimisticLineItems.map((lineItem) => (
            <li
              className="flex flex-col items-start md:items-center gap-x-5 gap-y-4 @container @sm:flex-row"
              key={lineItem.id}
            >
              <div className="relative aspect-square w-full max-w-24 overflow-hidden rounded-xl bg-[#161616] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-4">
                <Image
              ...
              <div className="flex flex-grow flex-col flex-wrap justify-between gap-y-2 @xl:flex-row">
                <div className="flex w-full flex-1 flex-col @xl:w-1/2 @xl:pr-4">
                  <span className="font-medium text-white">{lineItem.title}</span>
  ...
  return (
    <form {...getFormProps(form)} action={action}>
      ...
      <div className="flex w-full flex-wrap items-center gap-x-5 gap-y-2">
        <span className="font-medium @xl:ml-auto text-white">{lineItem.price}</span>
        ...
        <div className="flex items-center rounded-lg border">
          ...
          <span className="flex w-8 select-none text-white justify-center focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">
            {lineItem.quantity}
          </span>
          <button
        ...
        <button
          ...
          value="delete"
        >
          <Trash2 size={20} color='#fff' strokeWidth={1} />
        </button>
      </div>
    </form>
  );
