      ...
      {(reviews) =>
      {
        ...
        return (
          <StickySidebarLayout
            sidebar={
              <>
                <Stream
                  fallback={
                    <div className="animate-pulse">
                      <h2 className="text-white mb-4 mt-0 text-xl font-medium @xl:my-5 @xl:text-2xl">
                        {reviewsLabel}
                      </h2>
                    </div>
                  }
                ...
                >
                  {(averageRating) => (
                    <>
                      <div className="mb-2 font-heading text-white text-5xl leading-none tracking-tighter @2xl:text-6xl">
                        {averageRating}
                      </div>
            ...
            sidebarSize="medium"
          >
            <div className="flex-1 border-t border-contrast-500">
              {reviews.map(({ id, rating, review, name, date }) =>
              {
                return (
                  <div className="border-b border-contrast-500 py-6" key={id}>
                    <Rating rating={rating} />
                    <p className="mt-5 text-lg font-semibold text-background">{name}</p>
                    <p className="mb-8 mt-2 leading-normal text-contrast-200">{review}</p>
                    <p className="text-sm text-contrast-200">{date}</p>
                  </div>
                );
              })}
    ...
    <StickySidebarLayout
      sidebar={
        <>
          ...
          <div className="text-white mb-2 font-heading text-white text-5xl leading-none tracking-tighter @2xl:text-6xl">
            0
          </div>
          <Rating rating={0} />
        </>
      ...
      sidebarSize="medium"
    >
      <div className="flex-1 border-t border-[#2e2e2e] py-12">
