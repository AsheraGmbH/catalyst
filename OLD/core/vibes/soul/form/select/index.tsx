  ...
  return (
    <div className={clsx('w-full space-y-2', className)}>
      {label !== undefined && label !== '' && (
        <Label className={clsx(hideLabel && 'sr-only', 'mb-2', hideLabelMobile && 'hidden md:block')} htmlFor={id}>
          {label}
        </Label>
      )}
            ...
            <SelectPrimitive.Viewport>
              {options.map((option) => (
                <SelectPrimitive.Item
                  className="w-full cursor-default select-none rounded-xl px-3 py-2 text-sm font-medium text-contrast-400 outline-none transition-colors hover:bg-contrast-400 hover:text-foreground focus-visible:bg-contrast-100 focus-visible:text-foreground data-[state=checked]:text-background @4xl:text-base"
                  key={option.value}
                  onMouseEnter={() =>
                  {
