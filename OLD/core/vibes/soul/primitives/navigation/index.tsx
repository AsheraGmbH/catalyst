...
)
{
  ...
  return (
    ...
    >
      <div
        className={clsx(
          // NOTE: `relative` is important so the Popover.Anchor has a stable containing block + width.
          'relative flex items-center justify-between gap-1 bg-[var(--nav-background,hsl(var(--background)))] py-2 pl-3 pr-2 transition-shadow @4xl:rounded-2xl @4xl:px-2 @4xl:pl-6 @4xl:pr-2.5',
          isFloating
      ...
      >
        ...
        <Popover.Root onOpenChange={setIsMobileMenuOpen} open={isMobileMenuOpen}>
          {/* Give Anchor an explicit measurable box; keep old alignment/width behavior */}
          <Popover.Anchor asChild>
  	      <div className="pointer-events-none absolute left-0 right-0 top-full h-px w-full" />
	  </Popover.Anchor>
...
<Popover.Content
  ...
  className="
    ...
    w-[min(calc(100vw-16px),var(--radix-popover-trigger-width))]
...
>
        ...
        >
          {searchAction ? (
            <Popover.Root onOpenChange={setIsSearchOpen} open={isSearchOpen}>
              {/* Measurable full-width anchor + old alignment/size behavior */}
              <Popover.Anchor asChild>
  		  <div className="pointer-events-none absolute left-0 right-0 top-full h-px w-full" />
	      </Popover.Anchor>
  ...
  className="
    z-[60]
    max-h-[calc(var(--radix-popover-content-available-height)-16px)]
    w-[min(calc(100vw-16px),var(--radix-popover-trigger-width))]
    max-w-[calc(100vw-16px)]
    py-2 @container
    data-[state=open]:animate-in data-[state=closed]:animate-out
...
>
      ...
      <div className="perspective-[2000px] absolute left-0 right-0 top-full z-50 hidden w-full justify-center @4xl:flex">
  ...
  return (
    <>
      ...
      >
        ...
        />
        <input
          className="flex-grow bg-transparent pl-2 text-white text-lg font-medium outline-0 focus-visible:outline-none @xl:pl-0"
...
})
{
  ...
  const changeCurrencyToIframe = (currency: string) =>
  {
    const iframe = document.getElementById('ashera-embed') as HTMLIFrameElement;
    if (!iframe) return;
    iframe?.contentWindow?.postMessage({ type: 'change_currency', currency });
        ...
        >
          {currencies.map((currency) => (
            <DropdownMenu.Item
              ...
              onSelect={() =>
              {
                changeCurrencyToIframe(currency.id);
                startTransition(async () =>
                {
                  const formData = new FormData();
