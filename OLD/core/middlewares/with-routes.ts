...
const isAnalyzePath = (p: string) => p === '/analyze' || p.startsWith('/analyze/');
const looksLikeLocaleAnalyzePath = (p: string) =>
  // e.g. /en/analyze/client.html, /de/analyze/...
  /^\/[a-z]{2}(?:-[A-Z]{2})?\/analyze(\/|$)/.test(p);
...
export const withRoutes: MiddlewareFactory = () => {
  return async (request, event) => {
    ...
    /**
     * RESERVED: /analyze/*
     * We want bundle-analyzer reports to be served as-is (no locale rewrite, no BC route rewrite),
     * otherwise you either get Catalyst chrome or a Next 404 after locale-prefixing.
     */
    if (isAnalyzePath(pathname)) {
      return NextResponse.next();
    }

    // If some earlier middleware prefixed the path with a locale (e.g. /en/analyze/...),
    // force it back to /analyze/... so it can hit app/analyze/[file]/route.ts.
    if (looksLikeLocaleAnalyzePath(pathname)) {
      const withoutFirstSegment = pathname.replace(/^\/[^/]+/, ''); // drop "/en"
      const url = new URL(withoutFirstSegment || '/', request.url);
    ...
    switch (node?.__typename) {
      ...
      default: {
        const { pathname: reqPathname } = new URL(request.url);

        const cleanPathName = clearLocaleFromPath(reqPathname, locale);
